...
| M14 | Analytics | Done: `QueryLog`/`KnowledgeGap`/`QuestionFrequency`/`Feedback` tables, ChatService and TestKnowledgeService instrumented (latency, tokens, estimated cost), `GET /analytics/overview`, `GET /analytics/knowledge-gaps`, `GET /analytics/questions` (top topics), `GET /analytics/sources` (top documents, aggregated from `MessageSource`), `POST /messages/{id}/feedback`, and the `/analytics` frontend dashboard (KPI cards, rate bars, ranked lists). Deferred: precise HF Inference Providers cost reconciliation (rough per-tier estimate only) |
